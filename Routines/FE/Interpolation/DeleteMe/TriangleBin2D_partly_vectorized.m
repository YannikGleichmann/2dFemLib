...
for k = 1:nTri
        ...
        in = inpolygon(p1_copy(:, 1), p1_copy(:, 2), poly(:, 1), poly(:, 2));
        P1 = p(t(k,1),:)';
        P2 = p(t(k,2),:)';
        P3 = p(t(k,3),:)';
        J_K = [P2 - P1, P3 - P1];
        p_loc = J_K\p1_copy(in, :)' - J_K\P1;
        for b = 1:nBasis
            fInter(in) = fInter(in) + fNodes(t(k, b)).*N{b}(p_loc(1, :)', p_loc(2, :)');
        end
        p1_copy(in, :) = ones(nnz(in), 2).*[inf, inf];  % ganz zum schluss
end
fInter == f(p1(:, 1), p1(:, 2))